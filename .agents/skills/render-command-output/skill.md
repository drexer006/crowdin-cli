...
description: >-
  Use when adding or changing what a CLI command prints - creating or editing
  views (views.ts), list/item/grid/table rendering, deciding what --output
  plain, json, or toon emit, --verbose behavior, empt
---
...
* **Machine vs prose**: json, toon, _and_ plain are parseable contracts - the command owes the caller a result document, not narrative.
...
**toon - an AI agent.** The same data contract as json in TOON's token-efficient tabular encoding. Never diverges from json in _what_ is carried, only in how it's encoded (stderr records are blank-line separated).
